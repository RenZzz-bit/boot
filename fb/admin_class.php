...
Class Action {
	...
	public function signup() {
        ...
        if (isset($_POST['firstname'], $_POST['lastname'], $_POST['email'], $_POST['password'])) {
            ...
            // Optional: Handle file upload (Document Upload)
            $document = '';
            ...
            $query = "INSERT INTO users (firstname, lastname, email, password, gender, dob, document, verified) 
                      VALUES (?, ?, ?, ?, ?, ?, ?, 0)";  // 'verified' set to 0 initially (user is not verified)
            $stmt = $this->conn->prepare($query);
    ...
    private function uploadDocument($file) {
        ...
        if (!in_array($extension, $allowedExtensions)) {
            return "Invalid document format. Please upload a PDF or Word document.";
        }

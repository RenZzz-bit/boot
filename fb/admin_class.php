...
class Action
{
	...
	public function signup($firstname, $lastname, $email, $password, $gender, $month, $day, $year, $document)
	{
		...
		$query = "INSERT INTO users (firstname, lastname, email, password, gender, dob, document, date_created) 
              VALUES (?, ?, ?, ?, ?, ?, ?, NOW())"; // 8 placeholders
		$stmt = $this->conn->prepare($query);

		// Correct the bind_param to match the number of variables (8 variables)
		$stmt->bind_param("sssssss", $firstname, $lastname, $email, $hashedPassword, $gender, $dob, $document);
	...
	/* public function signup()
	{
		if (isset($_POST['firstname'], $_POST['lastname'], $_POST['email'], $_POST['password'])) {
			...
			if (isset($_FILES['document']) && $_FILES['document']['error'] === UPLOAD_ERR_OK) {
				...
				if (strpos($document, "Error") !== false) {
					return $document; // Return error message from uploadDocument
				}
			} else {
				return "Document upload failed. Please check the file.";
			}
	...
	private function uploadDocument($file)
	{
		...
		$targetDir = __DIR__ . '/../img/documents/';
		return "Target directory: " . $targetDir; // Debugging line
		if (!is_dir($targetDir)) {
		...
		if (move_uploaded_file($file['tmp_name'], $targetFile)) {
			...
			return "Error moving uploaded file. Check permissions and path.";
		}
	}
